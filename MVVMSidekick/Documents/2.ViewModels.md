...
>Command是用来与View中的事件和Button进行绑定的用户行为接收器，在View Model中一般被定义为属性成员。在使用中一般会同时跟随一个资源，但是在标准的 `ICommand` 接口中，这一特性并没有定义。 
...
> MVVM跟班儿项目提供了一种代理方式的，实现了`ICommand`，名为`CommandModel`的通用模型。 通过这个 `CommandMNodel<TCommand,TResource>` 泛型模型, 你可以定义一个实现了 `ICommand`的类型 和任意一种资源. 如果你同时在用别的框架，更喜欢用别的Command而不是`ReactiveCommane`, 在这里可以替换为你想要的类型。  


Think about image button, globalization button, and context menu cases, a `String` resource key is always necessary, so we provide code snippets like below.

> 在图形按钮，多语言的按钮，以及上下文菜单等用况里， string 的资源key是必要的。所以我们默认提供了下面的模板



In class file, type `propcmd` + `[Tab]` +`[Tab]`, following lines will appear.
>在类型文件中，键入`propcmd` +`[Tab]` +`[Tab]` ，编辑器将产生下面的代码
...
*You can modify property type and property name when the snippet highlight is still active and `[Tab]` again.*
>当你输入 `[Tab]`的时候，如果文字为高亮，你可以修改新建类型的名字和Command的名字。
...
A new view can be a control, a page or a window. It would be shown in place like desktop, frame or content control. The place to show a view is usually called **View Target**.  
>新视图可能是一个控件、一个页面或者一个窗体。它可能在桌面上、浏览框里或者在一个内容控件中显示。这种显示视图的目标，我们一般叫做**视图目标**


In MVVM-Sidekick we use `Stage` name the  **View Target** place, we can access `StageManager` property to show a new View Model in Command Logic.
