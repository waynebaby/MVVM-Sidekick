...
namespace MVVMSidekick.Views
{
    ...
    {
        private readonly ViewModelOptions pageViewModelCacheOptions;
        private readonly IServiceProvider serviceProvider;

        public Stage(NavigationManager navigationManager, ViewModelOptions pageViewModelCacheOptions, IServiceProvider serviceProvider)
        {
        ...
        {
            if (pageViewModelCacheOptions.vmRoutingTable.TryGetValue(typeof(TTarget), out string template))
            {
    ...
    public class ViewModelOptions
    {
        ...
        internal Dictionary<Type, string> vmRoutingTable = new Dictionary<Type, string>();
        public ViewModelOptions(IServiceCollection services)
        {
            this.services = services;
        }
        public ViewModelOptions AddModel<TViewModel>() where TViewModel : class, IViewModelWithPlatformService
        {
            services.AddSingleton<TViewModel>();
            var ag = typeof(TViewModel)?.GetGenericArguments()[1];
            var route = ag.GetCustomAttribute(typeof(RouteAttribute), true) as RouteAttribute;
            vmRoutingTable[typeof(TViewModel)] = route.Template;
            return this;
        }
